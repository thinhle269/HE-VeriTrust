...
def flatten_state(state: Dict[str, torch.Tensor]) -> np.ndarray:
    return np.concatenate([t.detach().cpu().numpy().astype(np.float32).reshape(-1) for t in state.values()])
...
def unflatten_state(vec: np.ndarray, schema) -> Dict[str, torch.Tensor]:
    ...
    for name, shape in schema:
        ...
        out[name] = torch.from_numpy(np.asarray(vec[off:off + n], dtype=np.float32).reshape(shape).copy())
        off += n
    if off != vec.size:
...
class FederatedClient:
    ...
    def __init__(self, client_id: int, X: torch.Tensor, y: torch.Tensor, device: torch.device, lr: float=0.002, local_epochs: int=2, batch_size: int=256, optimizer: str='adam', grad_clip_norm: float=1.0, max_update_norm: float=10.0, class_weight: Optional[torch.Tensor]=None, label_smoothing: float=0.0, balanced_sampler: bool=False, is_malicious: bool=False, attack: str='sign_flip', noise_sigma: float=1.0, num_classes: int=8, forge_attestation: bool=False, seed: int=0):
    ...
    def _loss_fn(self) -> nn.Module:
        return nn.CrossEntropyLoss(weight=self.class_weight, label_smoothing=self.label_smoothing)
    ...
    def train_round(self, global_model: nn.Module, global_flat: np.ndarray) -> ClientUpdate:
        local = copy.deepcopy(global_model).to(self.device)
        local.train()
        opt = torch.optim.Adam(local.parameters(), lr=self.lr) if self.optimizer_kind == 'adam' else torch.optim.SGD(local.parameters(), lr=self.lr, momentum=0.9)
        lf = self._loss_fn()
        loss_before = self._epoch_loss(local, lf)
        t0 = time.time()
        for _ in range(self.local_epochs):
            if self._sample_p is not None:
                idx = torch.multinomial(self._sample_p, self.n_samples, replacement=True)
            else:
        ...
        honest_delta = delta.copy() if self.is_malicious and self.forge_attestation else None
        if self.is_malicious:
            if self.attack == 'sign_flip':
                ...
                delta = delta + self._rng.normal(0.0, self.noise_sigma, size=delta.shape).astype(np.float32)
        ...
        return ClientUpdate(self.client_id, delta, self.n_samples, float(loss_before), float(loss_after), train_time, self.is_malicious, claimed, honest_delta)

    def _claimed_stats(self, delta, honest_delta, loss_before, loss_after) -> Dict[str, float]:
        denom = max(abs(loss_before), 1e-06)
        ...
        if self.is_malicious and self.forge_attestation and (honest_delta is not None):
            return {'cosine': float('nan'), 'loss_improvement': improvement, 'norm': float(np.linalg.norm(honest_delta)), 'n_samples': float(self.n_samples)}
        return {'cosine': float('nan'), 'loss_improvement': improvement, 'norm': float(np.linalg.norm(delta)), 'n_samples': float(self.n_samples)}
