...
def compute_metrics(y_true, y_pred, num_classes: int, loss: Optional[float]=None, label_names=None, with_confusion: bool=False) -> Dict:
    ...
    out = {'accuracy': float(accuracy_score(y_true, y_pred)), 'macro_f1': float(f1_score(y_true, y_pred, labels=labels, average='macro', zero_division=0)), 'weighted_f1': float(f1_score(y_true, y_pred, labels=labels, average='weighted', zero_division=0)), 'precision_macro': float(precision_score(y_true, y_pred, labels=labels, average='macro', zero_division=0)), 'recall_macro': float(recall_score(y_true, y_pred, labels=labels, average='macro', zero_division=0)), 'min_class_f1': float(np.min(per))}
...
def class_weights(counts: np.ndarray, mode: str='effective_number', beta: float=0.9999) -> Optional[np.ndarray]:
