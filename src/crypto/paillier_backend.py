...
class PublicContext:
    ...
    def encrypt_many(self, plaintexts: Sequence[int]) -> List[int]:
        ...
        res = Parallel(n_jobs=self.n_jobs, backend='loky')((delayed(_encrypt_chunk)(c, self.public_key) for c in chunks))
    ...
    def encrypt_batch(self, groups: Sequence[Sequence[int]]) -> List[List[int]]:
        ...
        if self.n_jobs == 1 or len(flat) < _PARALLEL_MIN:
            ...
            workers = max(1, joblib.cpu_count() if self.n_jobs < 0 else int(self.n_jobs))
...
class SecretContext:
    ...
    def decrypt_many(self, cts: Sequence[int], n_jobs: int=-1) -> List[int]:
        ...
        res = Parallel(n_jobs=n_jobs, backend='loky')((delayed(_decrypt_chunk)(c, self.private_key) for c in chunks))
