...
class ZeroTrustPolicy:
    ...
    def __init__(self, threshold: float=0.4, ema_beta: float=0.6, reject_decay: float=0.7, max_reject_streak: int=3, min_accept_fraction: float=0.5):
    ...
    def evaluate(self, client_ids: Sequence[int], trust: Sequence[float]) -> Decision:
        ...
        weights: Dict[int, float] = {}
        if accepted:
            adj = {c: max(smoothed[c], 0.001) for c in accepted}
            tot = sum(adj.values())
            weights = {c: v / tot for c, v in adj.items()}
        return Decision(accepted, rejected, weights, smoothed, {int(c): float(t) for c, t in zip(client_ids, trust)}, forced)

    def state(self) -> Dict[str, object]:
        return {'threshold': self.threshold, 'ema': dict(self._ema), 'streak': dict(self._streak)}

def quantise_weights(weights: Dict[int, float], order: Sequence[int], scale_bits: int=10, max_share: float=0.5) -> List[int]:
    ids = list(order)
    if not ids:
        return []
    w = np.array([max(float(weights.get(int(c), 0.0)), 0.0) for c in ids], dtype=np.float64)
    if w.sum() <= 0:
        w = np.ones(len(ids), dtype=np.float64)
    w = w / w.sum()
    cap = float(max_share)
    if len(ids) * cap < 1.0 - 1e-09:
        raise ValueError(f'max_share={cap} is unsatisfiable for {len(ids)} clients (needs >= {1.0 / len(ids):.3f})')
    for _ in range(64):
        over = w > cap + 1e-12
        if not over.any():
            break
