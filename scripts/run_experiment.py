...
def parse_args():
    ...
    p.add_argument('--attack', default=None, help='override experiments.attack.type')
    p.add_argument('--tag', default=None, help='suffix for the output directory')
    p.add_argument('--jobs', type=int, default=None, help='worker processes for Paillier (default: crypto.n_jobs). Set explicitly when running several seeds in parallel so the pools do not oversubscribe the CPU.')
    p.add_argument('--force-lock', action='store_true', help='take the results-directory lock even if another pid holds it')
...
def run_centralized(cfg, split, device, logger, seed: int) -> Dict:
    ...
    cw = class_weights(counts, cfg.training.get('class_weighting', 'none'), float(cfg.training.get('cb_beta', 0.9999)))
    w = torch.tensor(cw, device=device) if cw is not None else None
    lf = nn.CrossEntropyLoss(weight=w, label_smoothing=float(cfg.training.get('label_smoothing', 0.0)))
    ...
    for _ in range(int(cfg.federated.rounds)):
        ...
        model.eval()
        with torch.no_grad():
            pv = torch.cat([model(Xv[i:i + 16384]).argmax(1) for i in range(0, Xv.shape[0], 16384)]).cpu().numpy()
    ...
    model.eval()
    with torch.no_grad():
        pt = torch.cat([model(Xt[i:i + 16384]).argmax(1) for i in range(0, Xt.shape[0], 16384)]).cpu().numpy()
    test = compute_metrics(split.y_test, pt, split.num_classes, label_names=split.label_names, with_confusion=True)
    logger.info('[centralized seed=%d] test macro-F1=%.4f acc=%.4f', seed, test['macro_f1'], test['accuracy'])
    return {'test': test, 'state': model.state_dict()}

def build_clients(cfg, split, partition, device, malicious_flags, seed: int) -> tuple:
    counts = np.bincount(split.y_train, minlength=split.num_classes)
    cw = class_weights(counts, cfg.training.get('class_weighting', 'none'), float(cfg.training.get('cb_beta', 0.9999)))
    ...
    for cid, idx in partition.items():
        ...
        clients.append(FederatedClient(client_id=cid, X=X, y=y, device=device, lr=float(cfg.federated.client_lr), local_epochs=int(cfg.federated.local_epochs), batch_size=int(cfg.federated.local_batch_size), optimizer=str(cfg.federated.get('optimizer', 'adam')), grad_clip_norm=float(cfg.federated.get('grad_clip_norm', 1.0)), max_update_norm=float(cfg.federated.get('max_update_norm', 10.0)), class_weight=cw_t, label_smoothing=float(cfg.training.get('label_smoothing', 0.0)), balanced_sampler=bool(cfg.training.get('balanced_sampler', False)), is_malicious=bool(malicious_flags[cid]), attack=str(att.get('type', 'sign_flip')), noise_sigma=float(att.get('noise_sigma', 1.0)), num_classes=split.num_classes, forge_attestation=bool(att.get('forge_attestation', False)), seed=seed))
...
def main():
    ...
    lock = ResultsLock(out, label=f'run_experiment seeds={args.seeds}').acquire(force=args.force_lock)
    ...
    logger.info('data: train=%d val=%d test=%d features=%d classes=%d', len(split.y_train), len(split.y_val), len(split.y_test), split.num_features, split.num_classes)
    ...
    logger.info('scenarios=%s seeds=%s rounds=%d', wanted, seeds, cfg.federated.rounds)
    ...
    logger.info('model params=%d | encrypted dim=%d | packing: %s', n_params, dim, scheme.describe())
    logger.info('ciphertexts/client/round = %d (vs %d unpacked)', scheme.n_blocks(dim), dim)
    rows_round, rows_client, rows_seed, rows_pc = ([], [], [], [])
    for seed in seeds:
        logger.info('=' * 70)
        logger.info('SEED %d', seed)
        partition = dirichlet_partition(split.y_train, int(cfg.federated.num_clients), float(cfg.federated.dirichlet_alpha), seed)
        for name in wanted:
            if name == 'centralized':
                t0 = time.time()
                cr = run_centralized(cfg, split, device, logger, seed)
                rows_seed.append({'scenario': 'centralized', 'seed': seed, **_flat_test(cr['test'])})
                rows_pc += _pc_rows('centralized', seed, cr['test'])
                torch.save(cr['state'], ROOT / cfg.paths.models_dir / f'centralized_seed{seed}.pt')
            ...
            model = build_model(cfg, split.num_features, split.num_classes).to(device)
            clients, identities = build_clients(cfg, split, partition, device, flags, seed)
            pub = authority = None
            if bool(scen.get('crypto', False)) and bool(cfg.crypto.get('enabled', True)):
                ...
                dp = sk_cfg.dp
                authority = DecryptionAuthority(sec, scheme, AuthorityPolicy.from_config(cfg.crypto.authority), dim=dim, sketch_k=int(sk_cfg.get('k', 32)), sketch_density=float(sk_cfg.get('density', 0.5)), dp_enabled=bool(dp.get('enabled', True)), dp_relative_sigma=float(dp.get('sigma', 0.05)), clip_norm=float(cfg.federated.get('max_update_norm', 10.0)), sketch_mode=str(sk_cfg.get('mode', 'shifted')), n_jobs=int(cfg.crypto.get('n_jobs', -1)))
                for cid, ident in identities.items():
                    authority.enrol(cid, ident.public_bytes)
                logger.info('[%s] Paillier %d-bit keypair + authority ready (%.1fs)', name, cfg.crypto.key_size, time.time() - t0)
            engine = None
            ...
            server = FederatedServer(cfg, scen, model, clients, identities, (Xv, yv), (Xt, yt), split.num_classes, device, split.label_names, pub=pub, authority=authority, scheme=scheme, trust_engine=engine, logger=logger)
            t0 = time.time()
            reports = server.run()
            test = server.evaluate_final()
            logger.info('[%s seed=%d] %.1fs | test macro-F1=%.4f acc=%.4f', name, seed, time.time() - t0, test['macro_f1'], test['accuracy'])
            torch.save(server.model.state_dict(), ROOT / cfg.paths.models_dir / f'{name}_seed{seed}.pt')
            for rep in reports:
                rows_round.append({'scenario': name, 'seed': seed, **{k: v for k, v in rep.__dict__.items() if not isinstance(v, (dict, list))}})
                for pc in rep.per_client:
                    rows_client.append({'scenario': name, 'seed': seed, 'round_idx': rep.round_idx, **pc})
            rows_seed.append({'scenario': name, 'seed': seed, **_flat_test(test)})
            rows_pc += _pc_rows(name, seed, test)
            if authority is not None:
                pd.DataFrame(authority.audit_table()).to_csv(csv_dir / f'audit_{name}_seed{seed}.csv', index=False)
            _flush(csv_dir, rows_round, rows_client, rows_seed, rows_pc)
    summary = _summarise(csv_dir, rows_seed)
    logger.info('\n%s', summary.to_string(index=False))
    (out / 'config_used.json').write_text(json.dumps(cfg.to_plain(), indent=2, default=str), encoding='utf-8')
    lock.release()
    logger.info('done -> %s', out)

def _flat_test(t: Dict) -> Dict:
    return {'test_accuracy': t['accuracy'], 'test_macro_f1': t['macro_f1'], 'test_weighted_f1': t['weighted_f1'], 'test_min_class_f1': t.get('min_class_f1', float('nan')), 'test_loss': t.get('loss', float('nan'))}

def _pc_rows(scen: str, seed: int, t: Dict) -> List[Dict]:
    return [{'scenario': scen, 'seed': seed, 'class': k, 'f1': v} for k, v in (t.get('per_class_f1') or {}).items()]
...
def _summarise(csv_dir: Path, rows_seed) -> pd.DataFrame:
    ...
    g.columns = ['scenario' if a == 'scenario' else f'{a}_{b}' for a, b in g.columns]
