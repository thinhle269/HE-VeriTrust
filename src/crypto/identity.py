...
def _canonical(client_id: int, round_idx: int, ciphertexts: Sequence[int]) -> bytes:
    h = hashlib.sha256()
    h.update(_DOMAIN)
...
class ClientIdentity:
    ...
    def public_bytes(self) -> bytes:
        return self._sk.public_key().public_bytes(encoding=serialization.Encoding.Raw, format=serialization.PublicFormat.Raw)

    def sign_submission(self, round_idx: int, ciphertexts: Sequence[int]) -> bytes:
