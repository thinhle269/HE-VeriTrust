...
ABLATIONS = ['sketch_k', 'trust_threshold', 'dp_sigma', 'ema_alpha', 'evidence_alpha', 'evidence_z0', 'sketch_mode', 'malicious_fraction', 'dirichlet_alpha']
...
def wait_all(procs: List[subprocess.Popen], label: str) -> None:
    ...
    print(f'[{label}] finished in {(time.time() - t0) / 60:.1f} min' + (f'  ({len(bad)} of {len(codes)} failed - see runlogs/)' if bad else ''))

def stage_main(jobs: int) -> None:
    print('[main] 5 seeds in parallel')
    wait_all([launch(f'main_{s}', [PY, ROOT / 'scripts/run_experiment.py', '--seeds', s, '--jobs', jobs, '--tag', f's{s}']) for s in SEEDS], 'main')

def stage_ablation(jobs: int, seeds: Sequence[int], rounds: int) -> None:
    print(f'[ablation] {len(ABLATIONS)} sweeps in parallel, seeds={list(seeds)} rounds={rounds}')
    wait_all([launch(f'abl_{p}', [PY, ROOT / 'scripts/run_ablation.py', '--param', p, '--seeds', *[str(s) for s in seeds], '--rounds', rounds, '--jobs', jobs]) for p in ABLATIONS], 'ablation')

def stage_attacks(jobs: int, seeds: Sequence[int]) -> None:
    print('[attacks] coordinated-attack suite in parallel')
    procs = []
    for atk in ['sign_flip_scaled', 'ipm', 'alie', 'min_max', 'min_sum', 'block_evasion', 'unresolvable']:
        procs.append(launch(f'atk_{atk}', [PY, ROOT / 'scripts/run_experiment.py', '--attack', atk, '--tag', f'atk_{atk}', '--jobs', jobs, '--seeds', *[str(s) for s in seeds], '--scenarios', 'fedavg_attack', 'fedmedian', 'krum', 'bulyan', 'foolsgold', 'veritrust', 'veritrust_mamdani']))
...
def stage_forgery(jobs: int, seeds: Sequence[int]) -> None:
    ...
    wait_all([launch('forgery', [PY, ROOT / 'scripts/run_forgery_study.py', '--seeds', *[str(s) for s in seeds], '--jobs', jobs])], 'forgery')

def stage_privacy(trials: int) -> None:
    print('[privacy] leakage vs k, with DP epsilon')
    wait_all([launch('privacy', [PY, ROOT / 'scripts/run_privacy_eval.py', '--trials', trials])], 'privacy')
...
def stage_edgeiiot(jobs: int, seeds: Sequence[int]) -> None:
    ...
    wait_all([launch(f'edge_{s}', [PY, ROOT / 'scripts/run_experiment.py', '--config', cfg, '--seeds', s, '--jobs', jobs, '--tag', f'e{s}']) for s in seeds], 'edgeiiot')
...
def stage_tables() -> None:
    ...
    if list((ROOT / 'results').glob('e*_edgeiiot')):
        subprocess.run([PY, str(ROOT / 'scripts/analyze_results.py'), '--pattern', 'e*_edgeiiot', '--out', 'results/edgeiiot/tables'], check=False)
        subprocess.run([PY, str(ROOT / 'scripts/make_figures.py'), '--pattern', 'e*_edgeiiot', '--out', 'results/edgeiiot/figures'], check=False)

def main():
    ap = argparse.ArgumentParser()
    ap.add_argument('--stages', nargs='+', default=['main', 'privacy', 'forgery', 'attacks', 'ablation', 'edgeiiot', 'tables'])
    ap.add_argument('--skip-main', action='store_true')
    ap.add_argument('--jobs', type=int, default=4, help='Paillier workers per child process')
