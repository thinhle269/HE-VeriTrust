...
class MLP(nn.Module):
    ...
    def __init__(self, in_features: int, num_classes: int, hidden_sizes: Sequence[int]=(256, 128, 64), dropout: float=0.2, activation: str='relu'):
        super().__init__()
...
def build_model(cfg, in_features: int, num_classes: int) -> nn.Module:
    m = cfg.model
    return MLP(in_features, num_classes, hidden_sizes=list(m.hidden_sizes), dropout=float(m.dropout), activation=str(m.activation))
