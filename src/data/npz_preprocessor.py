...
class NpzPreprocessor:
    ...
    def __init__(self, cfg, project_root: Path):
        ...
        self.log = get_logger(f'data.{self.name}', self.root / cfg.paths.logs_dir, cfg.logging.level)

    def _fingerprint(self) -> str:
        payload = {'version': CACHE_VERSION, 'path': str(self.npz_path), 'cap': self.max_rows_per_class, 'scaler': self.scaler_name, 'test': self.test_size, 'val': self.val_size, 'seed': self.seed}
    ...
    def run(self, force: bool=False) -> Split:
        ...
        src_meta = json.loads(meta_src.read_text(encoding='utf-8')) if meta_src.exists() else {}
        ...
        label_names = [str(c) for c in names] if names and len(names) == len(enc.classes_) else [str(c) for c in enc.classes_]
        feature_names = src_meta.get('feature_names') or [f'f{i}' for i in range(X.shape[1])]
        idx = np.arange(len(y))
        strat = y if np.min(np.bincount(y)) >= 2 else None
        tr, te = train_test_split(idx, test_size=self.test_size, stratify=strat, random_state=self.seed)
        tr, va = train_test_split(tr, test_size=self.val_size / (1 - self.test_size), stratify=y[tr] if strat is not None else None, random_state=self.seed)
        ...
        split = Split(X_train=X_tr, y_train=y[tr], X_val=X_va, y_val=y[va], X_test=X_te, y_test=y[te], feature_names=list(feature_names), label_names=list(label_names), scaler=scaler)
        np.savez_compressed(npz_p, X_train=X_tr, y_train=y[tr], X_val=X_va, y_val=y[va], X_test=X_te, y_test=y[te])
        joblib.dump(scaler, sc_p)
        meta_p.write_text(json.dumps({'version': CACHE_VERSION, 'feature_names': list(feature_names), 'label_names': list(label_names), 'train_class_counts': split.class_counts(), 'shapes': {'train': list(X_tr.shape), 'val': list(X_va.shape), 'test': list(X_te.shape)}}, indent=2), encoding='utf-8')
        self.log.info('%s ready: train=%d val=%d test=%d features=%d classes=%d', self.name, len(split.y_train), len(split.y_val), len(split.y_test), split.num_features, split.num_classes)
