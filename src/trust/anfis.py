...
class ANFISHead(nn.Module):
    ...
    def __init__(self, n_inputs: int=3, n_mf: int=5, device: str='cpu'):
        ...
        lo, hi = (-1.0, 1.0)
        means = torch.stack([torch.linspace(lo, hi, self.n_mf) for _ in range(self.n_inputs)], dim=0)
        spread = (hi - lo) / max(self.n_mf - 1, 1)
        self.means = nn.Parameter(means)
        self.log_spreads = nn.Parameter(torch.full((self.n_inputs, self.n_mf), float(np.log(spread))))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(self.n_inputs):
            xi = x[:, i:i + 1]
            mu = torch.exp(-0.5 * ((xi - self.means[i].unsqueeze(0)) / (spreads[i].unsqueeze(0).abs() + _EPS)) ** 2)
            mus.append(mu)
        f = (mus[0].unsqueeze(2).unsqueeze(3) * mus[1].unsqueeze(1).unsqueeze(3) * mus[2].unsqueeze(1).unsqueeze(2)).reshape(x.shape[0], -1)
...
class NeuroFuzzyTrust:
    ...
    def __init__(self, n_mf: int=5, lr: float=0.02, device: str='cpu', seed: int=0):
    ...
    def fit(self, feats: Sequence[TrustFeatures], labels: Sequence[float], epochs: int=300) -> 'NeuroFuzzyTrust':
        X = torch.from_numpy(features_matrix(feats)).to(self.device)
        if X.shape[0] == 0:
            return self
        y = torch.tensor(np.asarray(labels, dtype=np.float32).reshape(-1), device=self.device)
...
class MamdaniTrust:
    _SETS = {'normalised': ((-1.0, -1.0, -0.76, -0.38), (-0.76, -0.38, 0.0, 0.46), (0.0, 0.46, 1.0, 1.0)), 'raw': ((-1.0, -1.0, -0.2, 0.2), (0.0, 0.3, 0.5, 0.8), (0.5, 0.8, 1.0, 1.0))}
    ...
    def score_many(self, feats: Iterable[TrustFeatures]) -> np.ndarray:
        ...
        for f in feats:
            ...
            num = 0.1 * untrusted + 0.45 * suspicious + 0.9 * trusted
...
def build_engine(cfg_trust, device: str='cpu', seed: int=0):
    ...
    if kind in ('none', 'off'):
        return None
    return NeuroFuzzyTrust(n_mf=int(cfg_trust.get('n_mf', 5)), lr=float(cfg_trust.get('lr', 0.02)), device=device, seed=seed)
