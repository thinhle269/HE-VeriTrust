...
class TrustFeatures:
    ...
    def as_array(self) -> np.ndarray:
        return np.array([self.proj_ref, self.norm_ratio, self.peer_agreement], dtype=np.float64)
...
def build_features(sketches: Dict[int, np.ndarray], reference_sketch: Optional[np.ndarray]=None) -> Dict[int, TrustFeatures]:
    ids = sorted(sketches)
    if not ids:
        return {}
