...
COORDINATED = {'alie', 'ipm', 'min_max', 'min_sum', 'block_evasion', 'unresolvable'}
...
def alie(benign: np.ndarray, n_total: int, n_malicious: int) -> np.ndarray:
    ...
    frac = float(np.clip((n_total - n_malicious - s) / denom, 1e-06, 1 - 1e-06))
...
def min_max(benign: np.ndarray, perturbation: str='std') -> np.ndarray:
    ...
    g = _search_gamma(lambda c: max((np.linalg.norm(c - b) for b in benign)), mu, p, thr)
...
def craft(method: str, benign: np.ndarray, n_total: int, n_malicious: int, epsilon: float=0.5, perturbation: str='std') -> np.ndarray:
    key = (method or '').lower()
    if key == 'alie':
    ...
    raise ValueError(f'unknown coordinated attack: {method}. Choose from {sorted(COORDINATED)}')

def apply_coordinated(updates: Sequence, method: str, epsilon: float=0.5, perturbation: str='std', slots: int=50, reference=None) -> int:
    ...
    if (method or '').lower() == 'block_evasion':
        ...
        return len(malicious)
    v = craft(method, np.stack(benign, axis=0), len(updates), len(malicious), epsilon, perturbation)
...
def unresolvable(reference: np.ndarray, target_norm: float, seed: int=0) -> np.ndarray:
