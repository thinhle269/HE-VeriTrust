...
from .packing import PackingScheme, assert_no_carry, block_widths, read_field, unpack_slot_sum
...
class ProbeSet:
    ...
    def probe_widths(self) -> np.ndarray:
        ...
        return np.array([w[list(self.pos[m])].sum() + w[list(self.neg[m])].sum() for m in range(self.k)], dtype=np.float64)
...
def probe_seed(round_idx: int, transcript_root: bytes, authority_nonce: bytes) -> bytes:
    h = hashlib.sha256()
...
def derive_probes(seed: bytes, dim: int, scheme: PackingScheme, k: int, density: float=0.5) -> ProbeSet:
    ...
    rng = np.random.default_rng(np.frombuffer(hashlib.sha512(seed).digest(), dtype=np.uint32))
    pos: List[Tuple[int, ...]] = []
    neg: List[Tuple[int, ...]] = []
...
def _probe_halves(cts: Sequence[int], pos: Sequence[int], neg: Sequence[int], nsquare: int, zero: int) -> Tuple[int, int]:
    cp = zero
    for b in pos:
...
def sketch_ciphertexts_parallel(pub: PublicContext, ciphertexts: Sequence[int], probes: ProbeSet, scheme: PackingScheme, n_jobs: int=-1) -> List[int]:
    if len(ciphertexts) != probes.n_blocks:
        raise ValueError(f'expected {probes.n_blocks} packed ciphertexts, got {len(ciphertexts)}')
    if probes.k == 0:
        return []
    ...
    nsq = pub.nsquare
    pairs = Parallel(n_jobs=n_jobs, backend='loky')((delayed(_probe_halves)(cts, probes.pos[m], probes.neg[m], nsq, pub.encrypt_zero()) for m in range(probes.k)))
    out: List[int] = []
    for cp, cn in pairs:
        out.append(cp)
        out.append(cn)
...
def sketch_ciphertexts_batch(pub: PublicContext, per_client: Sequence[Sequence[int]], probes: ProbeSet, scheme: PackingScheme, n_jobs: int=-1) -> List[List[int]]:
    for cts in per_client:
        if len(cts) != probes.n_blocks:
            raise ValueError(f'expected {probes.n_blocks} packed ciphertexts, got {len(cts)}')
    if probes.k == 0:
    ...
    pairs = Parallel(n_jobs=n_jobs, backend='loky')((delayed(_probe_halves)(cts_int[i], probes.pos[m], probes.neg[m], nsq, zero) for i, m in tasks))
...
def sketch_ciphertexts(pub: PublicContext, ciphertexts: Sequence[int], probes: ProbeSet, scheme: PackingScheme) -> List[int]:
    if len(ciphertexts) != probes.n_blocks:
        raise ValueError(f'expected {probes.n_blocks} packed ciphertexts, got {len(ciphertexts)}')
    out: List[int] = []
    for m in range(probes.k):
        assert_no_carry(scheme, max(len(probes.pos[m]), len(probes.neg[m])) or 1)
        cp = pub.add_many([ciphertexts[b] for b in probes.pos[m]]) if probes.pos[m] else pub.encrypt_zero()
        cn = pub.add_many([ciphertexts[b] for b in probes.neg[m]]) if probes.neg[m] else pub.encrypt_zero()
        out.append(cp)
        out.append(cn)
    return out

def decode_sketch(plaintexts: Sequence[int], probes: ProbeSet, scheme: PackingScheme, normalise: bool=True) -> np.ndarray:
...
def dp_epsilon(relative_sigma: float, delta: float=1e-05) -> float:
    if relative_sigma <= 0:
        return float('inf')
    return float(math.sqrt(2.0 * math.log(1.25 / float(delta))) / float(relative_sigma))

def dp_epsilon_composed(relative_sigma: float, rounds: int, delta: float=1e-05) -> float:
    if rounds <= 0:
        return 0.0
    eps = dp_epsilon(relative_sigma, delta / (2.0 * rounds))
    d_prime = delta / 2.0
    return float(math.sqrt(2.0 * rounds * math.log(1.0 / d_prime)) * eps + rounds * eps * (math.exp(eps) - 1.0))

def add_dp_noise(sketch: np.ndarray, sigma_abs: float, rng: Optional[np.random.Generator]=None) -> np.ndarray:
    if sigma_abs <= 0:
        return sketch
...
def project_plaintext(vec: np.ndarray, probes: ProbeSet) -> np.ndarray:
    ...
    block_sums = np.array([v[starts[b]:starts[b] + w[b]].sum() for b in range(probes.n_blocks)], dtype=np.float64)
