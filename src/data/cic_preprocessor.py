...
CACHE_VERSION = '2.1.0'
SUPERCLASS_RULES: List[Tuple[str, str]] = [('benign', 'Benign'), ('ddos-', 'DDoS'), ('dos-', 'DoS'), ('mirai-', 'Mirai'), ('recon-', 'Recon'), ('vulnerabilityscan', 'Recon'), ('dns_spoofing', 'Spoofing'), ('mitm-', 'Spoofing'), ('dictionarybruteforce', 'BruteForce'), ('backdoor_malware', 'Web'), ('browserhijacking', 'Web'), ('commandinjection', 'Web'), ('sqlinjection', 'Web'), ('uploading_attack', 'Web'), ('xss', 'Web')]
CLASS_ORDER = ['Benign', 'BruteForce', 'DDoS', 'DoS', 'Mirai', 'Recon', 'Spoofing', 'Web']
...
def _make_scaler(name: str):
    return {'standard': StandardScaler, 'minmax': MinMaxScaler, 'robust': RobustScaler}[str(name).lower()]()
...
class CicIotPreprocessor:
    ...
    def __init__(self, cfg, project_root: Path):
        ...
        self.log = get_logger('data.cic', self.root / cfg.paths.logs_dir, level=cfg.logging.level)

    def _fingerprint(self) -> str:
        payload = {'version': CACHE_VERSION, 'csv_root': str(self.csv_root), 'per_type_cap': self.per_type_cap, 'max_rows_per_class': self.max_rows_per_class, 'corr_threshold': self.corr_threshold, 'scaler': self.scaler_name, 'test_size': self.test_size, 'val_size': self.val_size, 'seed': self.seed}
        blob = json.dumps(payload, sort_keys=True).encode()
        return hashlib.sha256(blob).hexdigest()[:16]

    def _paths(self):
        fp = self._fingerprint()
        return (self.cache_dir / f'cic_{fp}.npz', self.cache_dir / f'cic_{fp}.meta.json', self.cache_dir / f'cic_{fp}.scaler.joblib')
    ...
    def run(self, force: bool=False) -> Split:
        ...
        if not force and npz_p.exists() and meta_p.exists() and sc_p.exists():
            ...
            return Split(feature_names=meta['feature_names'], label_names=meta['label_names'], scaler=joblib.load(sc_p), **arrays)
        ...
        del df
        idx = np.arange(len(y))
        tr, te = train_test_split(idx, test_size=self.test_size, stratify=y, random_state=self.seed)
        tr, va = train_test_split(tr, test_size=self.val_size / (1 - self.test_size), stratify=y[tr], random_state=self.seed)
        ...
        split = Split(X_train=X_tr, y_train=y[tr], X_val=X_va, y_val=y[va], X_test=X_te, y_test=y[te], feature_names=feat_cols, label_names=list(enc.classes_), scaler=scaler)
        np.savez_compressed(npz_p, X_train=X_tr, y_train=y[tr], X_val=X_va, y_val=y[va], X_test=X_te, y_test=y[te])
        joblib.dump(scaler, sc_p)
        meta_p.write_text(json.dumps({'version': CACHE_VERSION, 'fingerprint': self._fingerprint(), 'feature_names': feat_cols, 'label_names': list(enc.classes_), 'train_class_counts': split.class_counts(), 'shapes': {'train': list(X_tr.shape), 'val': list(X_va.shape), 'test': list(X_te.shape)}, 'provenance': provenance}, indent=2), encoding='utf-8')
    ...
    def _load_raw(self):
        if not self.csv_root.is_dir():
            raise FileNotFoundError(f'CIC-IoT CSV tree not found at {self.csv_root}. Set data.csv_root in the config.')
        ...
        for d in dirs:
            ...
            for f in files:
                if got >= self.per_type_cap:
                    break
                chunk = pd.read_csv(f, nrows=self.per_type_cap - got, low_memory=False)
                parts.append(chunk)
                got += len(chunk)
        ...
        self.log.info('Loaded %d rows / %d columns from %d attack types', len(df), df.shape[1], len(provenance))
    ...
    def _clean(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        self.log.info('Cleaning removed %d rows (non-finite + duplicates) -> %d', before - len(df), len(df))
        return df
    ...
    def _cap_per_class(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        self.log.info('After per-class cap %d: %d rows | %s', self.max_rows_per_class, len(out), dist)
        return out
    ...
    def _select_features(self, X_train: np.ndarray, cols: List[str]):
        ...
        keep &= ~const
        if const.any():
            self.log.info('Dropping %d constant features: %s', int(const.sum()), [cols[i] for i in np.where(const)[0]])
        ...
        if 0 < thr < 1:
            ...
            if len(idx) > 1:
                ...
                if drop:
                    ...
                    self.log.info('Dropping %d correlated features (|r|>=%.2f): %s', len(drop), thr, [cols[idx[j]] for j in sorted(drop)])
