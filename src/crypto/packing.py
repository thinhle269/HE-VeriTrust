...
class PackingScheme:
    ...
    def slots(self) -> int:
        ...
        if n < 1:
            raise ValueError(f'slot_bits={self.slot_bits} and shift_slots={self.shift_slots} leave no room in a {self.modulus_bits}-bit modulus')
        return int(n)
    ...
    def describe(self) -> dict:
        return {'modulus_bits': self.modulus_bits, 'value_bits': self.value_bits, 'headroom_bits': self.headroom_bits, 'slot_bits': self.slot_bits, 'slots_per_ciphertext': self.slots, 'shift_slots': self.shift_slots, 'max_total_weight': self.max_total_weight, 'scale': self.scale}
...
def assert_no_carry(scheme: PackingScheme, total_weight: int) -> None:
    ...
    if used >= bound:
        raise OverflowError(f'packing would carry between slots: total_weight={total_weight} needs {used.bit_length()} bits but slot_bits={scheme.slot_bits}. Reduce weight_scale_bits or raise headroom_bits (max_total_weight={scheme.max_total_weight}).')
...
def unpack(plaintexts: Sequence[int], scheme: PackingScheme, dim: int, total_weight: int) -> np.ndarray:
...
def unpack_slot_sum(plaintext: int, scheme: PackingScheme, n_real_coords: int) -> float:
    mask = (1 << scheme.slot_bits) - 1
    p = int(plaintext)
    total = 0
    for _ in range(scheme.slots):
...
def build_scheme(cfg_crypto) -> PackingScheme:
    ...
    return PackingScheme(modulus_bits=int(get('key_size', 2048)), value_bits=int(get('value_bits', 20)), headroom_bits=int(get('headroom_bits', 20)), scale=float(get('quantization_scale', 1000000.0)), shift_slots=int(sk_get('shift_slots', 0)))
