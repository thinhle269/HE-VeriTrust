...
def fedmedian(updates, weights=None) -> np.ndarray:
    return np.median(np.stack([u.astype(np.float64) for u in updates], 0), axis=0).astype(np.float32)
...
def _krum_scores(updates, f: int) -> np.ndarray:
    ...
    k = n - int(f) - 2
    if k < 1:
        warnings.warn(f'Krum undefined for n={n}, f={f} (needs n > 2f+2)', RuntimeWarning)
        k = 1
...
def multi_krum(updates, weights=None, num_byzantine: int=1, m: Optional[int]=None) -> np.ndarray:
    n = len(updates)
    if n == 1:
...
def foolsgold(updates, weights=None, kappa: float=1.0, eps: float=1e-05) -> np.ndarray:
    n = len(updates)
    if n == 1:
        return updates[0].astype(np.float32)
    U = np.stack([u.astype(np.float64).reshape(-1) for u in updates], 0)
...
_DISPATCH = {'fedavg': fedavg, 'median': fedmedian, 'fedmedian': fedmedian, 'trimmed_mean': trimmed_mean, 'multi_krum': multi_krum, 'krum': multi_krum, 'bulyan': bulyan, 'foolsgold': foolsgold}

def aggregate(method: str, updates, weights=None, **kw) -> np.ndarray:
    key = (method or 'fedavg').lower()
    if key not in _DISPATCH:
        raise ValueError(f'unknown aggregation method: {method}. Choose from {sorted(_DISPATCH)}')
    if weights is None:
