...
class ResultsLock:
    ...
    def acquire(self, force: bool=False) -> 'ResultsLock':
        ...
        if self.path.exists() and (not force):
            ...
            if _alive(pid):
                raise ResultsLocked(f"{self.path.parent} is already being written by pid {pid} ({info.get('label', '?')}, started {time.strftime('%H:%M:%S', time.localtime(info.get('t', 0)))}). Stop it first, or pass --force-lock.")
        self.path.write_text(json.dumps({'pid': os.getpid(), 'label': self.label, 't': time.time()}), encoding='utf-8')
        return self
