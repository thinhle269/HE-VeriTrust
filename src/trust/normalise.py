...
class EvidenceAccumulator:
    ...
    def __init__(self, alpha: float=0.4, z0: float=2.0, sigma: float=0.03) -> None:
    ...
    def update(self, feats: Dict[int, TrustFeatures]) -> Dict[int, TrustFeatures]:
        ...
        for cid, f in feats.items():
            ...
            self._state[cid] = cur
            out[cid] = TrustFeatures(proj_ref=self._squash(float(cur[0])), norm_ratio=float(np.clip(cur[1], -1.0, 1.0)), peer_agreement=self._squash(float(cur[2])))
        return out
