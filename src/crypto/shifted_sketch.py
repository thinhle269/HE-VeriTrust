...
def derive_shifted_probes(seed: bytes, dim: int, scheme: PackingScheme, k: int, density: float=0.5) -> ShiftedProbeSet:
    ...
    nb = len(widths)
    rng = np.random.default_rng(np.frombuffer(hashlib.sha512(_SHIFT_DOMAIN + seed).digest(), dtype=np.uint32))
    ...
    return ShiftedProbeSet(tuple(blocks), tuple(signs), fields, shift, int(dim), scheme.slots, widths)
...
def shifted_sketch_ciphertexts_batch(pub: PublicContext, per_client: Sequence[Sequence[int]], probes: ShiftedProbeSet, scheme: PackingScheme, n_jobs: int=-1) -> List[List[int]]:
    nsq = pub.nsquare
    sb = scheme.slot_bits
    deltas = list(probes.shift)
    if n_jobs == 1 or len(per_client) * probes.n_blocks < 256:
        shifted_all = [[_shift_block(c, d, sb, nsq) for c, d in zip(cts, deltas)] for cts in per_client]
    else:
        flat = [(i, b) for i in range(len(per_client)) for b in range(probes.n_blocks)]
        n_chunks = min(128, max(1, len(flat)))
        size = (len(flat) + n_chunks - 1) // n_chunks
        chunks = [flat[i:i + size] for i in range(0, len(flat), size)]
        res = Parallel(n_jobs=n_jobs, backend='loky')((delayed(_shift_chunk)([int(per_client[i][b]) for i, b in ch], [deltas[b] for _, b in ch], sb, nsq) for ch in chunks))
    ...
    for shifted in shifted_all:
        ...
        for m in range(probes.k):
            worst = max((max(len(a), len(b)) for a, b in (probes.contributors(m, t) for t in probes.fields)), default=1)
...
def decode_shifted_sketch(plaintexts: Sequence[int], probes: ShiftedProbeSet, scheme: PackingScheme) -> np.ndarray:
...
def project_plaintext_shifted(vec: np.ndarray, probes: ShiftedProbeSet) -> np.ndarray:
...
def spectral_norm(probes: ShiftedProbeSet) -> float:
    ...
    for i in range(k):
        for j in range(i, k):
            ...
            G[i, j] = G[j, i] = sum((v * large.get(key, 0.0) for key, v in small.items()))
